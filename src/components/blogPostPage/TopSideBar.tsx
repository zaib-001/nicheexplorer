import { getPostsTopSide } from "@/utils/sanityQueries";
import Link from "next/link";
import React from "react";

/**
 * Component for rendering the top sidebar
 */
export const TopSideBar = async () => {
  // Fetch top posts
  const postTop = await getPostsTopSide();
    ...
    <div className="flex min-w-full flex-col gap-4 rounded-md border-t-[5px] border-solid border-gray-900 bg-gray-50 p-4 sm:min-w-full">
      ...
      <div>
        <ul className="flex list-disc flex-col gap-2 pl-4">
          {postTop.map((post: any, index: any) => (
            <li
              key={index}
